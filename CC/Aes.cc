...
vector<byte> Aes::aes_encrypt(byte *v, const byte keyS[]){
    AES_KEY key_struct;
...
vector<byte> Aes::aes_decrypt(byte *v, const byte keyS[]){
    AES_KEY key_struct;
...
vector<byte> Aes::aes_128_ECB_en(const vector<byte>& plainData, const byte keyS[]){
...
vector<byte> Aes::aes_128_ECB_de(const vector<byte>& v, const byte keyS[]){
    AES_KEY key_struct;
...
vector<byte> Aes::aes_128_CBC_en(vector<byte>& plainData, const byte keyS[], vector<byte>& iv){
    ...
    for(int i = 0; i < out.size(); i += 16){
        carrier = Xor::block_xor(&out[i], &carrier[0]);
...
vector<byte> Aes::aes_128_CBC_de(vector<byte>& v, const byte keyS[], vector<byte>& iv){
    vector<byte> b;
    vector<byte> out (v.size());
    for(int i = v.size() - 16; i >= 16; i -= 16){
        b = aes_decrypt(&v[i], keyS);
        b = Xor::block_xor(&v[i - 16], &b[0]);
        for(int j = 0; j < 16; ++j)
            out[i + j] = b[j];
    }
    b = aes_decrypt(&v[0], keyS);
    b = Xor::block_xor(&iv[0], &b[0]);
    for(int i = 0; i < 16; ++i)
        out[i] = b[i];
    remove_padding(out);
    return out;
...
vector<byte> Aes::aes_128_CBC_de_NP(vector<byte>& v, const byte keyS[], vector<byte>& iv){
    vector<byte> b;
    vector<byte> out (v.size());
    for(int i = v.size() - 16; i >= 16; i -= 16){
        b = aes_decrypt(&v[i], keyS);
...
vector<byte> Aes::aes_128_CTR(const vector<byte>& v, const byte keyS[], const vector<byte>& nonce){
    vector<byte> ret(v.size());
    unsigned long int counter = 0;
    unsigned long int size = (unsigned long int)v.size()/16;
    if(v.size()%16 != 0) size++;
    for (counter = 0; counter < size; ++counter){
        vector<byte> run = Block::append_arrays(nonce, Conversion::intToByteArray(counter, false));
